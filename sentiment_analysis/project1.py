...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    pred=np.dot(feature_vector,theta)+theta_0
    loss=np.maximum(0,1-pred*label)
    return loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    loss_vec=np.maximum(0,1-pred*labels)
    loss=np.mean(loss_vec,axis=0)
    return loss
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    pred=(np.dot(feature_vector,current_theta)+current_theta_0)*label
    if pred<=0:
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    N,D=feature_matrix.shape[0],feature_matrix.shape[1]
    theta=np.zeros(D,)
    theta_0=float(0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(N):
            ...
            sum_theta+=theta
            sum_theta_0+=theta_0
    return sum_theta/(T*N),sum_theta_0/(T*N)
    raise NotImplementedError
